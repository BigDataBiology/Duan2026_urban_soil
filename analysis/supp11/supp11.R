...
result_shan <- map_dfr(seq_along(samples_shan), function(k) {
  combos <- combn(samples_shan, k, simplify = FALSE)
  if (length(combos) > 10) {
    combos <- sample(combos, 10)}
  
  map_dfr(combos, function(city_set) {
    centroids <- unique(
      genes$centroid[
        genes$sample %in% city_set &
          genes$City == "Shanghai"])
    tibble(
      n_samples = k,
      sample_list = paste(city_set, collapse = ", "),
...
result_nan <- map_dfr(seq_along(unique(samples_nan)), function(k) {
  ...
  map_dfr(combos, function(city_set) {
    centroids <- unique(
      genes$centroid[genes$sample %in% city_set ])
    tibble(
      n_samples = k,
      sample_list = paste(city_set, collapse = ", "),
      n_centroids = length(centroids)
...
regression_plot <- rbind(result_shan %>% mutate(City = "Shanghai"),
                         result_nan %>% mutate(City = "Nanjing")) %>% 
  ggplot(aes(x = n_samples, y = n_centroids, color = City)) +
  geom_point() +
  geom_smooth(method = "lm") +
  stat_poly_eq(aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")),
               parse = TRUE) +
  scale_color_manual(values = pal_8, labels = function(x) gsub(" ", "\n", x)) +
  theme_minimal() +
  xlab("Number of metagenomic samples") +
  ylab("Number of ARGs") +
  labs(fill = "") + 
...
ggsave("supp11/supp11.svg", regression_plot + 
         theme(axis.text.x = element_blank(),
               plot.margin = margin(0, 0, 0, 0), panel.spacing = unit(5, "mm")) , width = 100, height = 80, unit = "mm")
